...
class GatewayDto:
    def __init__(self):
        ...
        # Sensor unit section
        self.mainCirkulationPumpTemp: int = 0
        self.boilerTemperature: int = 0
        self.solarCollectorTemp: int = 0
        self.coolantTemperature: int = 0
        self.lightLevel: int = 0
        self.auxAdcValue: int = 0
    ...
    def canMsgParse(self, can_message):
        # Відомі CAN ID
        known_ids = [0x069, 0x247, 0x033, 0x043, 0x301, 0x070]
    
        if(can_message.can_id == 0x069):
            import time
            self.sensorUnitMsgTimestamp = int(time.time() * 1000)
            self.mainCirkulationPumpTemp = can_message.data[0]
            self.boilerTemperature = can_message.data[1]
            self.solarCollectorTemp = can_message.data[3]
            self.coolantTemperature = can_message.data[2]
            self.lightLevel = can_message.data[4]
            self.auxAdcValue = can_message.data[5]
            #print(f"Sensor Unit: {self.mainCirkulationPumpTemp}, {self.boilerTemperature}, {self.solarCollectorTemp}, {self.coolantTemperature}, {self.lightLevel}, {self.auxAdcValue}")
